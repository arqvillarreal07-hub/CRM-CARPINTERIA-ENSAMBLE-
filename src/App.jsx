...
export default function App(){
  ...
  const modals= <div>
    ...
    {modal==="editMov"&&md&&<ModalW title="Editar Movimiento" onClose={cm}><EditMovForm key={md.id} mov={md} obras={obras} onSave={(nd,nm,np,no,nc)=>{if(md.t==="caja"){setCaja(prev=>prev.map(x=>x.id===md.cajaId?{...x,concepto:nd||x.concepto,monto:nm||x.monto,obra:no,resp:np||x.resp}:x));}else{const mNum=parseInt(md.id.replace('m',''));setMovs(prev=>prev.map(x=>{if(x.id===mNum){return {...x,desc:nd||x.desc,ing:md.t==="ing"?(nm||x.ing):0,egr:md.t!=="ing"?(nm||x.egr):0,prov:np,obra:no,cat:nc};}return x;}));}cm();show("Actualizado ✓");}} onDelete={()=>{if(confirm("¿Eliminar este movimiento?")){if(md.t==="caja"){setCaja(prev=>prev.filter(x=>x.id!==md.cajaId));}else{const mNum=parseInt(md.id.replace('m',''));setMovs(prev=>prev.filter(x=>x.id!==mNum));}cm();show("Eliminado");}}}/></ModalW>}
